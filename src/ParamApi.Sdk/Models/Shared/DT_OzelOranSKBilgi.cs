namespace ParamApi.Sdk.Models.Shared;

/// <summary>
/// Özel oran son kullanıcı bilgi modeli
/// Üye işyerinin müşterisine göstereceği komisyon oranlarını listeler
/// </summary>
public class DT_OzelOranSKBilgi
{
    /// <summary>
    /// Özel Oran SK Liste den gelen ID si
    /// </summary>
    public long Ozel_Oran_SK_ID { get; set; }

    /// <summary>
    /// Üye İşyerine ait Anahtar
    /// </summary>
    public string? GUID { get; set; }

    /// <summary>
    /// Sanal Pos numarası
    /// </summary>
    public int SanalPOS_ID { get; set; }

    /// <summary>
    /// Kredi Kartı Markası (Bonus, Axess vs)
    /// </summary>
    public string? Kredi_Karti_Banka { get; set; }

    /// <summary>
    /// Kredi Kartı Marka Görseli
    /// </summary>
    public string? Kredi_Karti_Banka_Gorsel { get; set; }

    /// <summary>
    /// Tek Çekim Oranı
    /// </summary>
    public string? MO_01 { get; set; }

    /// <summary>
    /// 2. Taksit Oranı
    /// </summary>
    public string? MO_02 { get; set; }

    /// <summary>
    /// 3. Taksit Oranı
    /// </summary>
    public string? MO_03 { get; set; }

    /// <summary>
    /// 4. Taksit Oranı
    /// </summary>
    public string? MO_04 { get; set; }

    /// <summary>
    /// 5. Taksit Oranı
    /// </summary>
    public string? MO_05 { get; set; }

    /// <summary>
    /// 6. Taksit Oranı
    /// </summary>
    public string? MO_06 { get; set; }

    /// <summary>
    /// 7. Taksit Oranı
    /// </summary>
    public string? MO_07 { get; set; }

    /// <summary>
    /// 8. Taksit Oranı
    /// </summary>
    public string? MO_08 { get; set; }

    /// <summary>
    /// 9. Taksit Oranı
    /// </summary>
    public string? MO_09 { get; set; }

    /// <summary>
    /// 10. Taksit Oranı
    /// </summary>
    public string? MO_10 { get; set; }

    /// <summary>
    /// 11. Taksit Oranı
    /// </summary>
    public string? MO_11 { get; set; }

    /// <summary>
    /// 12. Taksit Oranı
    /// </summary>
    public string? MO_12 { get; set; }

    /// <summary>
    /// Belirtilen taksit sayısının mevcut olup olmadığını kontrol eder
    /// -2 değeri taksit uygun değil anlamına gelir
    /// </summary>
    /// <param name="taksitSayisi">Kontrol edilecek taksit sayısı (1-12)</param>
    /// <returns>Taksit mevcut mu</returns>
    public bool IsTaksitAvailable(int taksitSayisi)
    {
        if (taksitSayisi < 1 || taksitSayisi > 12)
            return false;

        var oran = GetTaksitOrani(taksitSayisi);
        return !string.IsNullOrEmpty(oran) && oran != "-2.0000";
    }

    /// <summary>
    /// Belirtilen taksit sayısının oranını getirir
    /// </summary>
    /// <param name="taksitSayisi">Taksit sayısı (1-12)</param>
    /// <returns>Taksit oranı veya null</returns>
    public string? GetTaksitOrani(int taksitSayisi)
    {
        return taksitSayisi switch
        {
            1 => MO_01,
            2 => MO_02,
            3 => MO_03,
            4 => MO_04,
            5 => MO_05,
            6 => MO_06,
            7 => MO_07,
            8 => MO_08,
            9 => MO_09,
            10 => MO_10,
            11 => MO_11,
            12 => MO_12,
            _ => null
        };
    }

    /// <summary>
    /// Müsteri için uygun taksit seçeneklerini getirir (-2 değeri olanları filtreler)
    /// </summary>
    /// <returns>Uygun taksit sayıları listesi</returns>
    public List<int> GetAvailableTaksitOptions()
    {
        var taksitler = new List<int>();
        
        for (int i = 1; i <= 12; i++)
        {
            if (IsTaksitAvailable(i))
                taksitler.Add(i);
        }
        
        return taksitler;
    }

    /// <summary>
    /// Ödeme sayfasında gösterilecek taksit seçenekleri
    /// </summary>
    /// <returns>Taksit sayısı ve oran bilgisi listesi</returns>
    public List<(int TaksitSayisi, string Oran)> GetCustomerPaymentOptions()
    {
        var options = new List<(int, string)>();
        
        for (int i = 1; i <= 12; i++)
        {
            var oran = GetTaksitOrani(i);
            if (!string.IsNullOrEmpty(oran) && oran != "-2.0000")
            {
                options.Add((i, oran));
            }
        }
        
        return options;
    }
} 